 ...
 * 
 * As per the principle, the functions that use references to the base classes must be able to use objects of 
 * derived class without knowing it.
 * Thus, in the example shown below, the function calculateArea which uses the reference of "Rectangle" should be 
 * able to use the objects of 
...
public class LSPDemo {
    ...
    * 
    * In case, we try to establish ISA relationship between Square and Rectangle such that we call "Square is a 
    * Rectangle", 
    * below code would start behaving unexpectedly if an instance of Square is passed
    * Assertion error will be thrown in case of check for area and check for breadth, although the program will 
    * terminate as
